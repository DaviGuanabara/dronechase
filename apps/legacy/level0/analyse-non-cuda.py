...
from loyalwingmen.modules.environments_pyflyt.level0.pyflyt_level0_environment import (
    PyflytL0Enviroment as Level0_env,
)
...
def on_avaluation_step():
    ...
    rl_frequency = 30
    env_fns = [
        lambda: Level0_env(GUI=True, rl_frequency=rl_frequency) for _ in range(n_envs)
    ]
    vectorized_environment = VecMonitor(SubprocVecEnv(env_fns))  # type: ignore
    vectorized_environment.reset()
    max_allowed_time = (
        1 / rl_frequency
...
def main():
    env = Level0_env(GUI=True, rl_frequency=30, debug=True)
