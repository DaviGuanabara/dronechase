...
class PyflytL2EnviromentModifiedV3(Env):
    """
    ...
    def __init__(
        ...
        self.CATCH_DISTANCE = 0.2  # 0.4  # 0.2
        ...
        # pursuer.lidar.radius = 2 * dome_radius

        pursuer1 = self.quadcopter_manager.get_pursuers()[1]
        pursuer1.set_munition(0)
        pursuer1.drive(np.array([0, 0, 0, 1]))
    ...
    def reset(self, seed=0):
        ...
        attitude = np.zeros(3)
        self.quadcopter_manager.replace_invader(invader, position, attitude)

        pursuer = self.quadcopter_manager.get_pursuers()[0]
        position = np.random.uniform(-1, 1, 3)
        attitude = np.zeros(3)
        self.quadcopter_manager.replace_quadcopter(pursuer, position, attitude)
        pursuer.set_munition(0)

        # pursuer_extra = self.quadcopter_manager.get_pursuers()[1]
        # position = np.random.uniform(-1, 1, 3)
        # attitude = np.zeros(3)
        # self.quadcopter_manager.replace_quadcopter(pursuer_extra, position, attitude)
    ...
    def replace_invader_if_close(self):
        ...
        if distance < self.CATCH_DISTANCE:  # type: ignore
    ...
    def compute_reward(self):
        ...
        if distance < self.last_distance:  # type: ignore
        ...
        if distance > self.dome_radius:  # type: ignore
            penalty += self.MAX_REWARD

        # if self.step_calls > self.max_step_calls:
        #    penalty += self.MAX_REWARD

        return score + bonus - penalty  # type: ignore
