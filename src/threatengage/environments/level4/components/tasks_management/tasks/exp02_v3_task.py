...
class Exp02_V3_Task(Task):
    ...
    def init_globals(self):
        ...
        self.last_closest_to_origin_position = self.dome_radius
        self.current_closest_to_origin__position = self.dome_radius
    ...
    def compute_reward(self, successful_shots: int = 0, pursuers_exploded: int = 0):
        ...
        closest_invader_to_origin_id = (
            self.offset_handler.identify_closest_invader_to_origin()
        )

        if closest_invader_to_origin_id > -1:
            closest_invader_to_origin: Quadcopter = (
                self.entities_manager.get_quadcopters(id=closest_invader_to_origin_id)[
                    0
                ]
            )

            invader_position = closest_invader_to_origin.inertial_data["position"]

            if np.linalg.norm(self.last_closest_to_origin_position) == self.dome_radius:
                self.last_closest_to_origin_position = invader_position

            current_distance = float(np.linalg.norm(invader_position - position))
            last_distance = float(
                np.linalg.norm(self.last_closest_to_origin_position - position)
            )
            # = self.dome_radius
            # self.current_closest_to_origin__position = self.dome_radius

            score -= float(np.linalg.norm(invader_position - position))
