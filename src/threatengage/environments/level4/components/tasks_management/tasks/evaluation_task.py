...
from ...utils.normalization import normalize_inertial_data

"""
Exp04_Task is a task that uses the KamikazeNavigator to control the Loitering Munition and 
uses RL agent to control the first pursuer. The second LW is stopped.
...
[x] - Corrigir o Shoot de tal forma que retorne quem atirou no kamikaze, um loyalwingman ou o agent
[x] - Reward de 50% ou 80% quando o lw matar um kamikaze
[x] - Remover a area protegida do termination e do reward
[x] - Setu Loyalwingmen BT
"""


class Evaluation_Task(Task):
    def __init__(
        self, entities_manager: EntitiesManager, dome_radius: float, configuration: Dict
    ):
        print("Evaluation - Air Combat - init")
        print("Reward will be deactivated")
        print("All informations will be passed through info dict.")
    ...
    def _process_configuration(self, configuration: Dict):
        # Each driver can be Neural Network Agent or a Behavior Tree
        self.drivers_configuration = configuration.get("drivers", [])
        self.NUM_PURSUERS = len(self.drivers_configuration)

        # Defaut Values
        self.munition_per_defender = configuration.get("munition_per_defender", 20)
        ...
        self.ENEMY_BORN_RADIUS = configuration.get("ENEMY_BORN_RADIUS", 6)
        self.INITIAL_ROUND = configuration.get("INITIAL_ROUND", 1)

        self.SHOW_PURSUER_NAME = configuration.get("SHOW_NAME", False)

        self.STEP_INCREMENT_FROM_CONFIG = configuration.get("STEP_INCREMENT", 100)
        self.MAX_STEP_FROM_CONFIG = configuration.get("MAX_STEP", 300)
        self.TIME_IS_LIMITED_FROM_CONFIG = configuration.get("TIME_IS_LIMITED", False)
    ...
    def init_constants(self):
        ...
        self.STEP_INCREMENT = (
            self.STEP_INCREMENT_FROM_CONFIG
        )  # 100  # Increment 6 seconds in time for each kill
        self.MAX_STEP = (
            self.MAX_STEP_FROM_CONFIG
        )  # 300  # 300 calls = 20 seconds for rl_frequency = 15

    def init_globals(self):
        self.current_step = 0
        self.current_round = self.INITIAL_ROUND
        # self.lw_kills = np.zeros(self.NUM_PURSUERS)
        # self.lw_alive = np.ones(self.NUM_PURSUERS)
    ...
    def drive_lw(self):
        ...
        for pursuer in pursuers:
            driver = self.drivers.get(f"{pursuer.id}", None)

            if driver is None:
                continue

            elif hasattr(driver, "predict"):
            ...
            elif hasattr(driver, "update"):
                driver.update(pursuer, self.offset_handler)

            else:
                driver(pursuer)
                # pursuer.drive(driver

            if self.SHOW_PURSUER_NAME:
                pursuer.show_name()

    # def update_model_path(self, model_path):
    #    self.lw_driver = PPO.load(model_path)

    def compute_lw_observation(self, pursuer: Quadcopter) -> Dict:
    ...
    def on_episode_start(self):
        ...
        for pursuer in pursuers:
            self.lw_kills[f"{pursuer.id}"] = 0
            self.lw_alive[f"{pursuer.id}"] = True
            self.lw_munitions[f"{pursuer.id}"] = self.munition_per_defender
    ...
    def process_explosion_range_invaders(self) -> Tuple[int, int, int]:
        ...
        for pursuer_id in list(identified_invaders_nearby.keys()):
            ...
            # self.lw_alive[f"pursuer_id"] = False
    ...
    def process_shoot_range_invaders(self) -> Tuple[int, int]:
        ...
        for pursuer_id in list(identified_invaders_nearby.keys()):
            ...
            # self.lw_munitions[f"{pursuer_id}"] -= 1
            if were_successful:
                self.lw_kills[f"{pursuer_id}"] += 1
    ...
    def compute_termination(self) -> bool:
        ...
        if self.current_step > self.MAX_STEP and self.TIME_IS_LIMITED_FROM_CONFIG:
    ...
    def compute_info(self):
        ...
        pursuers = self.entities_manager.get_armed_pursuers()
        for pursuer in pursuers:
            # self.lw_alive[f"{pursuer.id}"] = pursuer.armed
            # self.lw_munitions[f"{pursuer.id}"] = pursuer.gun.munition

            info[f"{pursuer.quadcopter_name}"] = {}
            info[f"{pursuer.quadcopter_name}"]["lw_kills"] = self.lw_kills[
                f"{pursuer.id}"
            ]
            info[f"{pursuer.quadcopter_name}"][
                "lw_alive"
            ] = pursuer.armed  # self.lw_alive[f"{pursuer.id}"]
            info[f"{pursuer.quadcopter_name}"][
                "lw_munitions"
            ] = pursuer.gun.munition  # self.lw_munitions[f"{pursuer.id}"]
            info[f"{pursuer.quadcopter_name}"]["current_wave"] = self.current_round
            info[f"{pursuer.quadcopter_name}"]["step"] = self.current_step

        return info
        # return {
        #    "lw_kills": self.lw_kills,
        #    "lw_alive": self.lw_alive,
        #    "lw_munitions": self.lw_munitions,
    ...
    def get_pursuers_names(self):
        name_key = "name"
        return [
            f"{self.drivers_configuration[i][name_key]}"
            for i in range(self.NUM_PURSUERS)
        ]

    def spawn_pursuer_squad(self):
        positions = self.generate_positions(self.NUM_PURSUERS, 2)
        pursuers_names = self.get_pursuers_names()
        pursuers = self.entities_manager.spawn_pursuer(
            positions, pursuers_names, lidar_radius=2 * self.dome_radius
        )
        ...
        for i, pursuer in enumerate(pursuers):
            ...
            if self.drivers_configuration[i]["type"] == "nn":
                self.drivers[f"{pursuer.id}"] = PPO.load(
                    self.drivers_configuration[i]["path"]
                )
            elif self.drivers_configuration[i]["type"] == "bt":
                self.drivers[f"{pursuer.id}"] = self.loyalwingman_navigator

            else:

                self.drivers[f"{pursuer.id}"] = lambda pursuer: pursuer.drive(
                    np.array([0, 0, 0, 1])
                )
    ...
    # ===============================================================================
    # Normalize. This should be in another file
    # ===============================================================================
