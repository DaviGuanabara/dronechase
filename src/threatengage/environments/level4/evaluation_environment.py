...
class EvaluationEnvironment(Env):
    ...
    def __init__(
        ...
        self.init_components(dome_radius, configuration, GUI)
    ...
    def init_globals(self):
        ...
        if hasattr(self, "lines"):
            for pursuer_id in self.lines:
                for line in self.lines[pursuer_id]:
                    self.simulation.removeUserDebugItem(line)

        self.lines = {}

    def init_components(self, dome_radius, configuration, GUI):
        ...
        self.task_progression = TaskProgression(
            TasksDispatcher.evaluation_vFinal(
                self.dome_radius, self.entities_manager, configuration
            )
        )
    ...
    def step(self, actions_not_used):
        """
        This is a step for evaluation environment. So all the entities are controlled by the task class,
        inclusing neural network based ones.
        """
    ...
    def update_last_positions(self, entity: Quadcopter):
        ...
        if variation < 1:
            ...
            self.last_positions[entity.id] = position

        else:

            for line in self.lines[entity.id]:
                line_color = (
                    [1, 0, 0]
                    if entity.quadcopter_type == EntityType.LOITERINGMUNITION
                    else [0, 0, 1]
                )

                self.simulation.addUserDebugLine(
                    position,
                    position,
                ...
                self.last_last_positions[entity.id] = position
                self.last_positions[entity.id] = position
    ...
    def draw_trajectory(self):
        ...
        entities = self.entities_manager.get_all()
        # entities = self.entities_manager.get_all_armed()
        for entity in entities:
            if not entity.armed and entity.id in self.lines.keys():
                pass
                # for line in self.lines[entity.id]:
                #   self.simulation.removeUserDebugItem(line)
                ...
                self.update_last_positions(entity)
                ...
                historic: list = self.lines.get(entity.id, [])

                if len(historic) > 80:
                    first_int = historic.pop(0)

                    line = self.simulation.addUserDebugLine(
                        self.last_last_positions[entity.id],
                        self.last_positions[entity.id],
                        lineColorRGB=line_color,
                        lineWidth=2,
                        lifeTime=0,
                        replaceItemUniqueId=first_int,
                    )
                ...
                self.lines[entity.id] = historic

                # if len(historic) > 200:
                #    self.simulation.removeUserDebugItem(historic.pop(0))
