...
from loyalwingmen.entities.quadcoters.fake_quadcopter import (
    FakeQuadcopter,
    Quadcopter,
    EntityType,
)
...
from loyalwingmen.entities.navigators.loyalwingman_navigator import (
    LoyalWingmanBehaviorTree,
    SacrificeAttack,
    MoveToFormation,
    ChaseThreat,
)
...
def test_chase_threat():
    print("test_chase_threat")
    building_position = np.array([0, 0, 1])
    # Make fake Kamikaze
    lm_positions = np.array([[0, 0, 10]])
    lw_positions = np.array([[0, 5, 5]])
    ...
    navigator = LoyalWingmanBehaviorTree(building_position)
    loyalwingman = lws[0]
    navigator.update(loyalwingman, offset_handler)

    assert isinstance(
        navigator.current_tree_node, ChaseThreat
    ), f"State {type(navigator.current_tree_node).__name__} is not {ChaseThreat.__name__}"
    print(type(navigator.current_tree_node).__name__)


def test_move_to_formation():
    print("test_move_to_formation")
    building_position = np.array([0, 0, 1])
    # Make fake Kamikaze
    lm_positions = np.array([[0, 0, 10]])
    lw_positions = np.array([[0, 5, 5]])
    ...
    lws, lms = create_fake_drones(lw_positions, lm_positions)
    loyalwingman: FakeQuadcopter = lws[0]
    loyalwingman.insert_gun_availability(False)
    # Initialize OffsetHandler and Navigator
    offset_handler = OffsetHandler(10)
    offsets = gen_offset_from_fakes(lws, lms)
    offset_handler.insert_offset(offsets)
    ...
    assert isinstance(
        navigator.current_tree_node, MoveToFormation
    ), f"State {type(navigator.current_tree_node).__name__} is not {MoveToFormation.__name__}"
    print(type(navigator.current_tree_node).__name__)


def test_sacriface_attack():
    print("test_sacriface_attack")
    building_position = np.array([0, 0, 1])
    # Make fake Kamikaze
    lm_positions = np.array([[0, 0, 10]])
    lw_positions = np.array([[0, 5, 5]])
    ...
    lws, lms = create_fake_drones(lw_positions, lm_positions)
    loyalwingman: FakeQuadcopter = lws[0]
    loyalwingman.insert_gun_munition(0)
    loyalwingman.insert_gun_availability(False)
    # Initialize OffsetHandler and Navigator
    offset_handler = OffsetHandler(10)
    offsets = gen_offset_from_fakes(lws, lms)
    ...
    assert isinstance(
        navigator.current_tree_node, SacrificeAttack
    ), f"State {type(navigator.current_tree_node).__name__} is not {SacrificeAttack.__name__}"
    print(type(navigator.current_tree_node).__name__)


# test_chase_threat()
# test_move_to_formation()
# test_sacriface_attack()
