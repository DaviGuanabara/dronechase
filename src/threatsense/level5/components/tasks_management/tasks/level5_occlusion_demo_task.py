...
class Level5_Occlusion_Demo_Task(Task):
    def __init__(
        self,
        # simulation,
    ...
    def spawn_invader_squad(self):
        positions = np.array([
            [1.5, 2.0, 2.0],  # Enemy 1 - barrier
            [-1.5, 2.0, 2.0],  # Enemy 2 - barrier
            [0.0, 2.0, 2.0],  # Enemy 3 - barrier
            [0.5, 3.0, 2.0],  # Enemy 4 - behind
            [-0.5, 3.0, 2.0],  # Enemy 5 - behind
            [4.0, 3.0, 2.0],  # Enemy 6 - optional decoy
            [-4.0, 3.0, 2.0],  # Enemy 7 - optional decoy
            [6.0, 2.5, 2.0],  # Enemy 8 - optional decoy
            [-6.0, 2.5, 2.0],  # Enemy 9 - optional decoy
            [0.0, 5.0, 2.0],  # Enemy 10 - far away
            [5.0, 5.0, 2.0],  # Enemy 11 - far away
            [-5.0, 5.0, 2.0],  # Enemy 12 - far away
        ])
        ...
        # Arma apenas os primeiros 5
        for invader in invaders[5:]:
            self.entities_manager.disarm_by_quadcopter(invader)
    ...
    def spawn_pursuer_squad(self):
        ...
        agent = self.entities_manager.spawn_pursuer(
            positions=positions[:1], names=["Agent"], lidar_radius=2 * self.dome_radius, use_fused_lidar=True,
        )[0]

        allies = self.entities_manager.spawn_pursuer(
            positions[1:], [f"Ally{i}" for i in range(1, self.NUM_PURSUERS)], lidar_radius=2 * self.dome_radius, use_fused_lidar=True,
        )
        ...
        self.entities_manager.set_agent(drone_id=agent.id)
