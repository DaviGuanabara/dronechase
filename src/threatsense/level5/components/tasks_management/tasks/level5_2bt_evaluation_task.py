...
print("[ThreatSense] Importing Level 5 Fusion Task")

"""
O ambiente vai começar com 10 aliados e 5 inimigos.
Os inimigos serão incrementados de 5 em 5 a cada round, para assim em pouco tempo
chegar a 30 inimigos, com uma alta probabilidade de oclusão.
"""


class Level52BTEvaluationTask(Task):
    def __init__(
        self,
        # simulation,
    ...
    def init_constants(self):
        ...
        self.NUM_PURSUERS = 1 + 1  # +1 for the agent 'Behavior Tree'
        ...
        # Remove pressure for killing invaders quickly
        # In this case, kept like 10 kills time.
        self.STEP_INCREMENT = 100
        self.MAX_STEP = 300 + 10 * self.STEP_INCREMENT
    ...
    def init_globals(self):
        ...
        self.kills_per_drone = {
            pursuer.id: {"name": pursuer.quadcopter_name, "type": "BT", "kills": 0}
    ...
    def process_shoot_range_invaders(self) -> None:
        identified_invaders_nearby = self.offset_handler.identify_invaders_in_range(
        ...
        for pursuer_id in list(identified_invaders_nearby.keys()):
            if were_successful := self.entities_manager.shoot_by_ids(
                ...
                self.kills_per_drone[pursuer_id]["kills"] += 1
    ...
    def compute_reward(
        ...
        """
        EVALUATION TASK DO NOT USES REWARD
        """
    ...
    def compute_info(self):
        ...
        return {
            "kills_per_drone": self.kills_per_drone,
            "deads": self.deads,
    ...
    def spawn_invader_squad(self):
        # num_invaders = 2
        positions = self.generate_positions(
            self.MAX_NUM_INVADERS, self.ENEMY_BORN_RADIUS)
        # np.array([[0, 0, 10]])  #   # /2
    ...
    def spawn_pursuer_squad(self):
        ...
        allies = self.entities_manager.spawn_pursuer(
            positions, ally_names, lidar_radius=2 * self.dome_radius, use_fused_lidar=True
        )

        for ally in allies:
            ally.set_munition(self.MUNITION_PER_DEFENDER)
