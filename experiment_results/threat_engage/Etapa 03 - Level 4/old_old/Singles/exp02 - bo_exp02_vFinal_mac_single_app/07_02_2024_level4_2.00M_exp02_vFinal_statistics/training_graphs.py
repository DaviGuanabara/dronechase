...
for trial_path in trials_path:
    ...
    try:
        ...
        if (
            "rollout/ep_rew_mean" in training_data.columns
            and "time/total_timesteps" in training_data.columns
        ):
            filtered_data = training_data[
                training_data["time/total_timesteps"] >= 32768
            ]
            filtered_data = filtered_data.drop_duplicates(
                subset=["time/total_timesteps"]
            )
            filtered_data_filled = filtered_data.fillna(
                filtered_data.rolling(5, min_periods=1).mean()
            )

            all_data.append(
                filtered_data_filled.set_index("time/total_timesteps")[
                    "rollout/ep_rew_mean"
                ]
            )
            print(f"Processed {trial_path}")
        else:
            print(f"Required columns are missing in {trial_path}, skipping this file.")
