"""Spawn a single drone on x=0, y=0, z=1, with 0 rpy."""
import numpy as np

# from PyFlyt.core import aviary_loyalwingmen
# from PyFlyt.core.aviary_loyalwingmen import Aviary

from loyalwingmen.modules.environments_pyflyt.level0.pyflyt_level0_simulation_functional import (
    AviarySimulation as Aviary,
)


# the starting position and orientations
start_pos = np.array([[0.0, 0.0, 1.0]])
start_orn = np.array([[0.0, 0.0, 0.0]])

# environment setup
env = Aviary(render=True)
# env = AviarySimulation(render=True)

# set to position control
env.set_mode(7)
# env.set_all_setpoints(np.array([[1.0, 1.0, 0.0, 1.0]]))
print(env.get_loyalwingmen())
# simulate for 1000 steps (1000/120 ~= 8 seconds)
for i in range(1000):
    env.step(np.array([1.0, 1.0, 0.0, 1.0]))
