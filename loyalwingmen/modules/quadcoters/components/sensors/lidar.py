...
class LiDAR(Sensor):
    ...
    def _add_spherical(
        ...
        current_norm_distance = self.sphere[self.DISTANCE_CHANNEL_IDX][theta_point][phi_point]
    ...
    def _rotate_position(self, position: np.ndarray) -> np.ndarray:
        ...
        rotation_matrix = np.array(p.getMatrixFromQuaternion(quaternion)).reshape(3, 3).T
    ...
    def _normalize_phi(self, phi) -> float:
        return self._normalize_angle(
            phi,
            self.PHI_INITIAL_RADIAN,
            self.PHI_FINAL_RADIAN,
            self.n_phi_points
        )
