...
class ImmovableStructures:
    ...
    def spawn_building(simulation: BulletClient, position: list) -> ImmovableStructures:
        id = simulation.loadURDF(
            "cube_small.urdf",
            basePosition=position,
            baseOrientation=[0, 0, 0, 1],
        )

        Immovable_structures = ImmovableStructures(
            simulation, id, position, EntityType.PROTECTED_BUILDING
        )

        return Immovable_structures
    ...
    def spawn_ground(
        ...
        Immovable_structures = ImmovableStructures(
            simulation, id, position, EntityType.GROUND, dimensions
        )
    ...
    def update_color(self):
        ...
        if self.life / self.max_life > 0.9:
            ...
            p.changeVisualShape(self.id, -1, rgbaColor=yellow)
